...
int main(int argc, char *argv[]) {
  ...
  parser.setApplicationDescription("P2PTogether GUI");
  parser.addHelpOption();
  parser.addVersionOption();

  QCommandLineOption testModeOption("test-mode",
                                    "Enable test automation server");
  parser.addOption(testModeOption);

  QCommandLineOption testPortOption("test-port", "Test server port", "port",
                                    "19999");
  ...
  if (!QFileInfo::exists(peerServicePath)) {
    qCritical() << "Peer service executable not found at:" << peerServicePath;
  ...
  if (parser.isSet(testModeOption)) {
    quint16 tPort = parser.value(testPortOption).toUShort();
    qInfo() << "Test mode enabled via command line. Port:" << tPort;
    app.enableTestMode(tPort);
  }

...
void ControlStreamWorker::run() {
    qDebug() << "ControlStreamWorker::run() started in thread:"
    ...
    try {
        ...
        if (!stream_) { // Check if stream creation failed
            ...
            qWarning() << "ControlStreamWorker::run() - stream_ is null after "
                          "openControlStream";
            return;
        }
        ...
        qWarning() << "ControlStreamWorker::run() - Exception opening stream:"
                   << e.what();
        return;
    }
    ...
    if (quit_) {
        qDebug() << "ControlStreamWorker::run() - Exited Read loop because "
                    "quit_ is true.";
    } else if (!read_ok) {
        qDebug() << "ControlStreamWorker::run() - Exited Read loop because "
                    "stream_->Read() returned false.";
    }

    qDebug() << "ControlStreamWorker::run() - Calling stream_->Finish()...";
    grpc::Status st = stream_->Finish(); // This sends FIN to the server
    qDebug() << "ControlStreamWorker::run() - stream_->Finish() completed with "
                "status:"
...
void ControlStreamWorker::stop() {
    qDebug() << "ControlStreamWorker::stop() called in thread:"
             << QThread::currentThreadId();
    quit_ = true; // Signal the loop to exit
    if (ctx_) {
        qDebug()
            << "ControlStreamWorker::stop() - Calling ctx_->TryCancel()...";
        ctx_->TryCancel(); // Attempt to unblock Read() immediately
        qDebug() << "ControlStreamWorker::stop() - ctx_->TryCancel() called.";
    } else {
        qWarning()
            << "ControlStreamWorker::stop() - ctx_ is null, cannot cancel.";
    }
}

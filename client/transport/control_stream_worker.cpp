...
void ControlStreamWorker::run() {
    ...
    qInfo()
        << "ControlStreamWorker::run() - Exited gRPC Read loop. read_ok was "
        << read_ok;
...
void ControlStreamWorker::stop() {
    ...
    if (stream_) {
        qInfo()
            << "ControlStreamWorker::stop() - Calling stream_->WritesDone()...";
        if (!stream_
                 ->WritesDone()) { // Returns false if already finished or error
...
void ControlStreamWorker::send(const client::ClientMsg& msg) {
    // Can be very chatty during normal playback state updates.
    ...
    if (stream_) {
        // qInfo() << "ControlStreamWorker::send() - About to write to
        // stream...";
        bool ok = stream_->Write(msg, grpc::WriteOptions());
        // qInfo() << "ControlStreamWorker::send() - Write completed. Success:"
        // << ok;
        Q_UNUSED(ok);
    } else {
        qWarning()

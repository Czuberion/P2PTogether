...
void createMenus(QMainWindow* window, gui::App* app,
    ...
    QObject::connect(
        createSessionAction, &QAction::triggered, [window, worker, app]() {
            ...
            if (app && !app->getCurrentSessionId().isEmpty() &&
                app->getCurrentSessionId() != "default") {
            ...
            qInfo() << "GUI: CreateSession dialog exec() returned:"
                    << dialogResult << "(Accepted=" << QDialog::Accepted << ")";

            if (dialogResult == QDialog::Accepted) {
                qInfo() << "GUI: CreateSession dialog ACCEPTED. Reading input "
                           "values...";
                QString requestedSessionName = sessionNameInput->text();
                QString requestedUsername    = usernameInput->text();
                qInfo() << "GUI: Read values - sessionName:"
                ...
                QObject::connect(
                    ...
                    [requestedSessionName, window](const QString& sid,
                        ...
                        qInfo() << "GUI: sessionCreatedSuccessfully signal "
                                   "received!";
                ...
                QObject::connect(
                    ...
                    [window](const QString& errorMsg) {
                        qInfo()
                            << "GUI: sessionCreationFailed signal received!";
                ...
                worker->send(clientMsg);
                qInfo() << "GUI: worker->send() completed for "
                           "CreateSessionRequest.";
                // Response handled in App::onServerMessage
            } else {
                qInfo()
                    << "GUI: CreateSession dialog was cancelled or rejected.";
            }
    ...
    QObject::connect(
        leaveAction, &QAction::triggered, window, [window, worker, app]() {
            if (!app)
                return;
            QString sessionId = app->getCurrentSessionId();
            if (sessionId.isEmpty() || sessionId == "default") {
                QMessageBox::information(window, "No Active Session",
                                         "You are not currently in a session.");
                return;
            }
            if (QMessageBox::question(
                    window, "Leave Session",
            ...
            if (worker) {
                client::ClientMsg msg;
                auto* req = msg.mutable_leave_session_request();
                req->set_session_id(sessionId.toStdString());
                req->set_hlc_ts(QDateTime::currentMSecsSinceEpoch());
                worker->send(msg);
            }

            app->leaveSessionLocal();
        });
    ...
    auto updateSessionActions = [app, roleStore, createSessionAction,
        ...
        if (!app)
            return;
        QString sessionId  = app->getCurrentSessionId();
        bool sessionActive = !sessionId.isEmpty() && sessionId != "default";

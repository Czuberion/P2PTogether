...
App::App(quint16 grpcPort, QObject *parent)
    : // QObject(parent), m_grpcPort(grpcPort), m_lastAppliedPlaybackHlc(-1) {
      QObject(parent), m_grpcPort(grpcPort), m_localUsername(""),
      m_lastAppliedPlaybackHlc(-1), m_playlistOriginSec(0.0),
      m_activePlaylistSequenceId(0), m_playbackCompletedCurrentStream(false) {
...
void App::onServerMessage(const client::ServerMsg &msg) {
  ...
  case client::ServerMsg::kRoleDefinitionsUpdate: {
    m_roleStore->processRoleDefinitionsUpdate(msg.role_definitions_update());
    break;
  }
  ...
  case client::ServerMsg::kLocalPeerIdentity: {
    ...
    qInfo() << "GUI: Received LocalPeerIdentity for peer:" << receivedPeerId;
    ...
    if (msg.local_peer_identity().has_username()) {
      ...
      if (m_roleStore->getLocalPeerId() ==
        ...
        m_localUsername = username; // Update App's direct m_localUsername too
  ...
  case client::ServerMsg::kPlaybackStateCmd: {
    ...
    if (cmd.stream_sequence_id() != m_activePlaylistSequenceId) {
      qInfo() << "Ignoring PlaybackStateCmd because its stream_sequence_id ("
    ...
    if (cmd.hlc_ts() > m_lastAppliedPlaybackHlc) {
      ...
      if (m_playPauseBtn) {
        ...
        m_playPauseBtn->setToolTip(cmd.target_is_playing() ? "Pause" : "Play");
      }
      // if (m_seekSlider)
  ...
  case client::ServerMsg::kPlaylistReset: {
    ...
    qInfo() << "App::onServerMessage: PROCESSING PlaylistReset (Type 10):"
            ...
            << resetCmd.start_pts() << "HLC" << resetCmd.hlc_ts() << "OriginSec"
            << resetCmd.origin_sec();
    ...
    qInfo() << "App::onServerMessage: After PlaylistReset: m_playlistOriginSec="
            << m_playlistOriginSec
  ...
  case client::ServerMsg::kCreateSessionResponse: {
    ...
    if (resp.has_error_message()) {
      ...
      emit sessionCreationFailed(QString::fromStdString(resp.error_message()));
      emit sessionStateChanged(false);
      qInfo() << "App::onServerMessage - emitted sessionCreationFailed and "
      ...
      emit sessionCreatedSuccessfully(
          QString::fromStdString(resp.session_id()),
          QString::fromStdString(resp.invite_code()));
      qInfo() << "App::onServerMessage - emitted sessionCreatedSuccessfully";
      emit sessionStateChanged(true);
      qInfo() << "App::onServerMessage - emitted sessionStateChanged(true)";
    }
    qInfo()
        << "App::onServerMessage - kCreateSessionResponse handling complete";
  } break;
  case client::ServerMsg::kJoinSessionResponse: {
    ...
    if (resp.has_error_message()) {
      ...
      if (resp.has_session_name()) {
        sessionNameFromJoinResp = QString::fromStdString(resp.session_name());
      }

      if (!resp.session_snapshot().empty()) {
        client::p2p::SessionStateData session_state_data;
        if (session_state_data.ParseFromString(resp.session_snapshot())) {
            ...
            for (const auto &identity :
              ...
              if (m_roleStore->getLocalPeerId() ==
                ...
                if (identity.has_username()) {
                  m_localUsername = QString::fromStdString(identity.username());
                  qInfo() << "GUI: My username set to:" << m_localUsername
                          << "from join snapshot.";
                }
              }
            }
      ...
      showSessionInfoDialog(
          ...
          QString::fromStdString(resp.session_id()), sessionNameFromJoinResp,
  ...
  case client::ServerMsg::kChatMsg: {
    ...
    bool is_self =
        (!local_id_from_store.isEmpty() && sender_id == local_id_from_store);
...
void App::setupP2P() {
  ...
  qInfo() << "App::setupP2P - Connected m_worker::finished to netThread::quit.";
...
void App::setupUI() {
  ...
  { // Scope for GrpcClient to get HLS port
    ...
    try {
      ...
      qInfo() << "Successfully fetched HLS port from peer service:" << hlsPort;
...
QString App::getCurrentSessionId() const { return m_currentSessionId; }

QString App::getCurrentInviteCode() const { return m_currentInviteCode; }
...
void App::onMpvPositionChanged(double newPosition) {
  ...
  qDebug() << "onMpvPositionChanged - Pos:" << newPosition
           ...
           << "MpvDuration:" << mpvDuration << "PausedByUser:" << isPausedByUser
           << "Buffering:" << isBuffering << "CoreIdle:" << coreIdle;
  ...
  if (coreIdle &&
    ...
    double durationToCompare =
        (mpvDuration > 1.0 && mpvDuration <= calculatedDurationFromSegs + 2.0)
            ? mpvDuration
    ...
    if (durationToCompare > 0) {
      if (isBuffering && (playbackTime >= durationToCompare - 1.0 ||
        ...
        qInfo() << "App::onMpvPositionChanged: Buffering near end of stream"

...
App::App(quint16 grpcPort,
         QObject* parent) : // QObject(parent), m_grpcPort(grpcPort),
...
void App::enableTestMode(quint16 port) {
...
void App::onServerMessage(const client::ServerMsg& msg) {
    ...
    case client::ServerMsg::kCreateSessionResponse: {
        qInfo() << "App::onServerMessage - Received kCreateSessionResponse";
        const auto& resp = msg.create_session_response();
        if (resp.has_error_message()) {
            qInfo() << "App::onServerMessage - Session creation FAILED:"
            ...
            qInfo()
                << "App::onServerMessage - emitted sessionCreationFailed and "
                   "sessionStateChanged(false)";
        } else {
            qInfo() << "App::onServerMessage - Session creation SUCCESS. ID:"
            ...
            // Notify test server BEFORE emitting signal that triggers blocking
            // dialog
            if (m_testServer) {
                QJsonObject eventData;
                eventData["session_id"] =
                    QString::fromStdString(resp.session_id());
                eventData["invite_code"] =
                    QString::fromStdString(resp.invite_code());
            ...
            emit sessionCreatedSuccessfully(
                ...
                QString::fromStdString(resp.invite_code()));
            qInfo()
                << "App::onServerMessage - emitted sessionCreatedSuccessfully";
            emit sessionStateChanged(true);
            qInfo()
                << "App::onServerMessage - emitted sessionStateChanged(true)";
        }
        qInfo() << "App::onServerMessage - kCreateSessionResponse handling "
                   "complete";
    } break;
    case client::ServerMsg::kJoinSessionResponse: {
        ...
        if (resp.has_error_message()) {
            ...
            // Notify test server if in test mode (before showing dialog which
            // blocks)
            if (m_testServer) {
                QJsonObject eventData;
                eventData["session_id"] =
    ...
    case client::ServerMsg::kChatMsg: {
        ...
        QString message_id =
            QString::fromStdString(chat_msg_proto.message_id());
        ...
        emit chatMessageReceived(message_id, sender_name_to_display,
                                 message_text, is_self);
    } break;
    case client::ServerMsg::kChatDeleteMsg: {
        const auto& delete_msg = msg.chat_delete_msg();
        QString message_id = QString::fromStdString(delete_msg.message_id());
        QString deleted_by = QString::fromStdString(delete_msg.deleted_by());
        emit chatMessageDeleted(message_id, deleted_by);
    } break;
    case client::ServerMsg::kPeerKicked: {
        const auto& kicked = msg.peer_kicked();
        QString peer_id    = QString::fromStdString(kicked.peer_id());
        QString local_id   = m_roleStore ? m_roleStore->getLocalPeerId() : "";

        if (!peer_id.isEmpty() && peer_id == local_id) {
            QString reason    = QString::fromStdString(kicked.reason());
            QString issued_by = QString::fromStdString(kicked.issued_by());
            QString detail =
                reason.isEmpty() ? "" : QString("\nReason: %1").arg(reason);
            QMessageBox::warning(
                m_mainWindow, "Removed from Session",
                QString("You were kicked by %1.%2").arg(issued_by, detail));
            resetSessionState();
            emit sessionStateChanged(false);
        } else if (m_roleStore) {
            m_roleStore->removePeer(peer_id);
        }
    } break;
    case client::ServerMsg::kPeerBanned: {
        const auto& banned = msg.peer_banned();
        QString peer_id    = QString::fromStdString(banned.peer_id());
        QString local_id   = m_roleStore ? m_roleStore->getLocalPeerId() : "";

        if (!peer_id.isEmpty() && peer_id == local_id) {
            QString reason    = QString::fromStdString(banned.reason());
            QString issued_by = QString::fromStdString(banned.issued_by());
            QString detail =
                reason.isEmpty() ? "" : QString("\nReason: %1").arg(reason);
            QMessageBox::critical(
                m_mainWindow, "Banned from Session",
                QString("You were banned by %1.%2").arg(issued_by, detail));
            resetSessionState();
            emit sessionStateChanged(false);
        } else if (m_roleStore) {
            m_roleStore->removePeer(peer_id);
    ...
    case client::ServerMsg::kPeerLeft: {
        const auto& left = msg.peer_left();
        QString peer_id  = QString::fromStdString(left.peer_id());
        if (m_roleStore)
            m_roleStore->removePeer(peer_id);
    } break;
    default:
...
void App::setupUI() {
    ...
    if (m_testModeEnabled) {
        ...
        qInfo() << "Test server listening on port"
                << m_testServer->serverPort();
...
void App::resetSessionState() {
    m_localUsername.clear();
    setActiveSessionDetails("default", "");
    ...
    if (m_roleStore)
        m_roleStore->clearAssignmentsAndUsers(true);

    if (m_roleStore)
        m_roleStore->setLocalDefaultRole("Admin");

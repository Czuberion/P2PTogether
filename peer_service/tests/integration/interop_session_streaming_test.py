...
def run_streaming_interop_test():
    ...
    regex_addr_strict = re.compile(
        r"Listening on address: /ip4/127\.0\.0\.1/tcp/(\d+)/p2p/([a-zA-Z0-9]+)"
    )
    regex_addr_loose = re.compile(
        r"Listening on address: .*/tcp/(\d+)/p2p/([a-zA-Z0-9]+)"
    )
    ...
    patterns = {"auth_success": False, "video_received": False, "received_seqs": set()}
    ...
    if not os.path.exists(video_path):
        print(
            f"WARNING: Video file not found at {video_path}. Testing might fail if ffmpeg fails."
        )
